...
public class CommandChatHitbox extends CommandBase {
	...
	public List<String> getCommandAliases() {
		return Arrays.<String> asList(new String[] { "h", "hb" });
	}
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length <= 0) {
			...
			try {
				...
				sender.addChatMessage(new TextComponentTranslation("commands.hitbox.exit.success"));
			} catch (Exception e) {
				e.printStackTrace();
				throw new WrongUsageException("commands.hitbox.exit.usage", new Object[0]);
			}
		} else if (args[0].equals("msg")) {
			ITextComponent ichatcomponent = getChatComponentFromNthArg(sender, args, 1, !(sender instanceof EntityPlayer));
			try {
				if (args[1].length() != 0) {
			...
			ITextComponent ichatcomponent = getChatComponentFromNthArg(sender, args, 2, !(sender instanceof EntityPlayer));
			try {
				if (args[1].length() != 0) {
					HitboxTVLogin.getChatSender().sendPrivateMessage(args[1], ichatcomponent.createCopy().getUnformattedText());
					sender.addChatMessage(new TextComponentTranslation("commands.hitbox.whisper.confirm", args[1]));
				}
			} catch (Exception e) {
			...
			try {
				if (args[1].length() != 0) {
					...
					sender.addChatMessage(new TextComponentTranslation("commands.hitbox.timeout.confirm", args[1], args[2]));
				}
			} catch (Exception e) {
			...
			try {
				if (args[1].length() != 0) {
					...
					sender.addChatMessage(new TextComponentTranslation("commands.hitbox.ban.confirm", args[1]));
				}
			} catch (Exception e) {
			...
			try {
				if (args[1].length() != 0) {
					...
					sender.addChatMessage(new TextComponentTranslation("commands.hitbox.ban-ip.confirm", args[1]));
				}
			} catch (Exception e) {
				throw new WrongUsageException("commands.hitbox.ban.usage", new Object[0]);
			}
			...
			try {
				if (args[1].length() != 0) {
					...
					sender.addChatMessage(new TextComponentTranslation("commands.hitbox.unban.confirm", args[1]));
				}
			} catch (Exception e) {
			...
			try {
				if (args[1].length() != 0) {
					...
					sender.addChatMessage(new TextComponentTranslation("commands.hitbox.add-modo.confirm", args[1]));
				}
			} catch (Exception e) {
			...
			try {
				if (args[1].length() != 0) {
					...
					sender.addChatMessage(new TextComponentTranslation("commands.hitbox.rm-modo.confirm", args[1]));
				}
			} catch (Exception e) {
			...
			try {
				if (args[1].length() != 0 && !args[1].equals("off")) {
					...
					sender.addChatMessage(new TextComponentTranslation("commands.hitbox.slowmode.confirm", args[1]));
				} else if (args[1].length() != 0 && args[1].equals("off")) {
					HitboxTVLogin.getChatSender().setSlowMode(Integer.parseInt("0"));
					sender.addChatMessage(new TextComponentTranslation("commands.hitbox.slowmode-off.confirm", args[1]));
				}
			} catch (Exception e) {
			...
			try {
				if (args[1].length() != 0 && args[1].equals("on")) {
					...
					sender.addChatMessage(new TextComponentTranslation("commands.hitbox.submode-on.confirm", args[1]));
				} else if (args[1].length() != 0 && args[1].equals("off")) {
					HitboxTVLogin.getChatSender().setSubscriberMode(false);
					sender.addChatMessage(new TextComponentTranslation("commands.hitbox.submode-off.confirm", args[1]));
				}
			} catch (Exception e) {
