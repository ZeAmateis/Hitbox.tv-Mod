...
public class GuiHitboxTVLogin extends GuiScreen {
	...
	protected void actionPerformed(GuiButton p_146284_1_) {
		...
		case 0:
			...
			break;
		case 1:
			this.mc.thePlayer.addChatMessage(new TextComponentTranslation("commands.hitbox.login.try"));
			try {
				if (HitboxTVCore.getData().isHitboxChannelNameCorrect(this.channelName.getText())) {
					...
					this.mc.displayGuiScreen(this.parentScreen);
					this.mc.thePlayer.addChatMessage(new TextComponentTranslation("channel.invalid"));
				}
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
		...
		case 2:
			try {
				...
				this.mc.thePlayer.addChatMessage(new TextComponentTranslation("commands.hitbox.exit.success"));
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				this.mc.displayGuiScreen(this.parentScreen);
