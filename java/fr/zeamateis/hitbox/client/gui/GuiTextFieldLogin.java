...
public class GuiTextFieldLogin extends Gui {
	...
	public GuiTextFieldLogin(FontRenderer p_i1032_1_, int p_i1032_2_, int p_i1032_3_, int p_i1032_4_, int p_i1032_5_) {
		...
		if (this.getVisible()) {
			...
			if (s.length() > 0) {
				...
				if (s1.matches(".*[a-z].*") || s1.matches(".*[A-Z].*") || s1.matches(".*[0-9].*") || s1.matches(".*[^a-zA-Z0-9].*")) {
					j1 = this.fontRenderer.drawStringWithShadow(s1.replaceAll(".", "*"), l, i1, i);
				}
			}
			...
			if (s.length() > 0 && flag && j < s.length()) {
				if (s.matches(".*[a-z].*") || s.matches(".*[A-Z].*") || s.matches(".*[0-9].*") || s.matches(".*[^a-zA-Z0-9].*")) {
					j1 = this.fontRenderer.drawStringWithShadow(s.substring(j).replaceAll(".", "*"), j1, i1, i);
				}
			}
